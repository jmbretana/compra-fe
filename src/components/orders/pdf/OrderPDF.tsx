...
import { Order, List } from "@interfaces";
...
const OrderPDF: React.FC<PdfPreviewProps> = ({ order }) => {
  const Title = () => {
    return (
        ...
        <View style={styles.flexColumn}>
          <View style={styles.formGridRow}>
            ...
            <Text style={styles.textImportant}>
              {capitalizeFirstLetter(order.proveedor)}
            </Text>
          </View>
          <View style={styles.formGridRow}>
            <Text style={styles.textLabel}>Fecha:</Text>
            <Text style={styles.textImportant}>{formatDate(order.fecha)}</Text>
          </View>
        </View>
      </View>
    </View>
  ...
  const TableHead = () => (
    <View style={{ width: "100%", flexDirection: "row", marginTop: 5 }}>
      ...
      <View style={[styles.theader, { width: "50px" }]}>
  ...
  const TableBody = () =>
    order.productsList.map((list: List) => {
      return (
